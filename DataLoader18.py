...
class DataLoader:
    ...
    def load_data(self):
        link_table = pd.read_csv('{loc}link table 2001-2018_csv.zip'.format(loc=self.data_location),
                                 header= 0,
                                 parse_dates=['datadate'],
                                 usecols=['datadate', 'GVKEY', 'LPERMNO', 'conm'])

        crsp = pd.read_csv('{loc}/crsp returns all 2001-2018_csv.zip'.format(loc=self.data_location),
                           header= 0,
                           parse_dates=['date'],
                           usecols=['date', 'PERMNO', 'PRC', 'RET', 'SHROUT', 'SHRCD', 'EXCHCD', 'PRIMEXCH', 'SHRCLS', 'TICKER'])

        compustat = pd.read_csv('{loc}compustat_fundamentals qtr 2001-2018_csv.zip'.format(loc=self.data_location),
                                header= 0 ,
                                parse_dates=['datadate'],
